...
public class World
{
	...
	public Avatar avatar      	= null;		    // Where the player is
	...
	private int light(int x, int y, int currentX, int currentY, double r)
	{
		...
		if(currentX < 0 || currentX >= tiles.length || currentY < 0 || currentY >= tiles[currentX].length)
			return 0;
		if (tiles[currentX][currentY].getSeen())
			return 0;
		if(tiles[currentX][currentY].isOpaque()) {
			tiles[currentX][currentY].setLit(true);
			return 0;
		}
		if(Math.sqrt(Math.pow((currentX - x), 2) + Math.pow((currentY - y), 2)) > r)
			return 0;
